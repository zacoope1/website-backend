...
mvn clean install && echo 'Running maven clean install.' && chmod +x mvnw && echo 'Setting permissions to mvnw' && screen -dmS website-backend bash -c './mvnw spring-boot:run; exec sh' && echo 'Created screen session' && echo 'Done!'
